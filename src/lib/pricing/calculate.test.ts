...
describe("calculatePriceCents", () => {
  ...
  it("rounds up to the nearest whole dollar across the 80/300 sqin boundary", () => {
    ...
    expect(just_below % 100).toBe(0);
    expect(just_above % 100).toBe(0);
    expect(just_above - just_below).toBeGreaterThanOrEqual(0);
    expect(just_above - just_below).toBeLessThanOrEqual(100);
  });

  it("rounds up to the nearest whole dollar across the 300/800 sqin boundary", () => {
    const w = 20;
...
describe("resolveAddOns", () => {
  ...
  it("dedupes repeated ids", () => {
    expect(resolveAddOns(["varnish", "varnish"], options)).toEqual([
      options[0],
    ]);
  });
  ...
  it("resolves multiple ids in order, skipping duplicates and unknowns", () => {
    expect(
      resolveAddOns(["stretched", "nonexistent", "varnish"], options),
...
describe("pricingConfig checkpoint prices", () => {
  it("prices 12x12 at $49", () => {
    expect(calculatePriceCents(12, 12, pricingConfig).priceCents).toBe(4900);
  });

  it("prices 24x24 at $145", () => {
    expect(calculatePriceCents(24, 24, pricingConfig).priceCents).toBe(14500);
  });

  it("prices 36x36 at $240", () => {
    expect(calculatePriceCents(36, 36, pricingConfig).priceCents).toBe(24000);
  });
});
