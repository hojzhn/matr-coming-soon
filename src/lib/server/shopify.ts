...
function shopifyCredentials(): { domain: string; token: string; version: string } {
...
export async function lookupDiscountCode(code: string): Promise<DiscountInfo> {
	...
	if (json.errors?.length) {
		console.error('Shopify discount lookup errors:', JSON.stringify(json.errors));
	...
	if (value?.__typename === 'DiscountPercentage') {
		return { title: discount.title, valueType: 'PERCENTAGE', value: value.percentage };
	}
	if (value?.__typename === 'DiscountAmount') {
		return { title: discount.title, valueType: 'FIXED_AMOUNT', value: Number(value.amount.amount) };
	}
	throw new Error("That discount code can't be applied here.");
}
...
export async function createDraftOrder(args: DraftOrderArgs): Promise<DraftOrderResult> {
	...
	const appliedDiscount = args.discount
		? {
				...
				value: args.discount.valueType === 'PERCENTAGE' ? args.discount.value * 100 : args.discount.value
			}
		: undefined;
